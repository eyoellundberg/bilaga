...
Priced transfers: `price_cents` on creation (requires `to` and an account). Downloads return 402 until the recipient pays via POST /api/inbox/{public_id}/pay, which settles in one D1 batch: mark paid, debit payer, credit seller minus FEE_BPS (500), ledger rows for payer, seller, and the `bilaga` fee account. Balances are integer cents on accounts; GET /api/balance shows the ledger; POST /api/credits (owner token) grants credit by email. Stripe top-ups and storage billing are unimplemented; storage is not charged. The public verification spec is at /verify; /api/receipt-key advertises `spec_url` and `retired_key_ids`.
