import { Paperclip, ArrowUpRight, Terminal } from 'lucide-react';
  ...
  return (
    <main className="shell">
      ...
      <article className="docs-content">
        ...
        <h1>
          One connection.
          <br />A simpler handoff.
        </h1>
        <p>
          Give your agent this website’s URL and a test token. It can read{' '}
          <a href="/llms.txt">llms.txt</a>, upload your file, and return a link
          you can share anywhere.
        </p>
        <p className="notice">
          Private preview · up to 1 GB per file · no charges. Test tokens come
          from the owner. Google sign-in and prepaid billing are coming next.
        </p>
        ...
        <p>
          ...
          <code>BILAGA_TOKEN</code> in your environment, then replace the origin
          and filename below.
        </p>
        <pre>{`python3 bilaga.py \\\n  --base https://YOUR-BILAGA-ORIGIN \\\n  --file report.pdf`}</pre>
        <p>
          The response includes the shareable link and expiry. Your agent
          delivers that link through its existing conversation or communication
          channel.
        </p>
        <h2>Check the handoff</h2>
        <pre>{`python3 bilaga.py --base https://YOUR-BILAGA-ORIGIN \\\n  --status TRANSFER_ID`}</pre>
        <p>
          Poll at most once every 15 seconds. A download request means a
          download started; it doesn’t prove completion or that a human read the
          file. Bilaga only marks a transfer “sent” when the agent reports
          delivery.
        </p>
        <h2>A small API, end to end</h2>
        <p>
          Send your token as <code>Authorization: Bearer …</code>. Keep it
          private: it can manage the transfers created with it. Recipient links
        ...
        <h2>Seven days to download</h2>
        <p>
          The clock starts after the upload completes. New downloads are blocked
          after seven days. Anyone with the link can download before then. The
          sender can revoke it earlier.
        </p>
        <p>
          In this preview, expired storage is cleared when another transfer is
          created or an owner runs cleanup. Automatic background cleanup,
          payment signals, and outbound webhooks are next steps. Unfinished
